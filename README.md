...
| **想走近了看、或者转个头看** | 视口是**第一人称透视相机**（不是等轴测托盘）：`WASD` 走（Shift 加速；抬头按 W 就是上升）、拖动**转头**（绕相机自己的轴，位置一动不动；方向是**画面跟手走**——往右拖 = 画面往右移 = 往左看）、滚轮改视场角变焦、双击回到自动取景。位置是真的——勾上「模型用这个机位」，模型就从你站的地方朝你看的方向拍（它那张仍是正交等轴测，见 `plan.md` D-76） |
