...
packages/
  ...
  i18n       文案层（zh-CN 是基准表，缺省语言是 en-US）
  cli        无头命令行
apps/
...
```
packages/
  core       voxel kernel · state encode/decode · geometry · world store · replay · orientation · linter
  mcai       .mcai container · conversation and screenshot archive · crash-recovery WAL
  render     software rasterizer · vanilla block model meshing · texture atlas · camera and overlays · orthographic ray pick · PNG codec
  tools      24 LLM tools · JSON Schema validation · document generation
  interop    .schem / .litematic / .obj · version migration
  agent      agent loop · completion gate · provider adapters and capability discovery
  i18n       the message layer (zh-CN is the base table; the default locale is en-US)
