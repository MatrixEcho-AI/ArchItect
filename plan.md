...
- **视口是**第一人称透视的自由相机（D-75 / D-76）**：`WASD` 平移（Shift 加速；
  W/S 沿视线前后，所以抬头按 W 就是上升），拖动是**原地转头**——角度绕相机自己的轴转、
  位置一动不动；滚轮改**视场角**（变焦，人不动），双击回到自动取景。
