...
> 状态：**P1–P4 已落地**（三层数据模型 · 记账 · `.mcai` 两个新条目 · `extra` 写回 ·
> 两种交换格式的四个字段 · 五个工具与闸门 · **两条软件渲染路径都能画实体**）。
> P5–P6 未实施（three.js 视口还没跟上，所以**桌面视口里看不到实体**）。
> 决策见 D-77…D-86。
...
| **P4 未做** | 旗帜图案、告示牌文字（见下面的记录） | 这两样是"方块实体里唯一需要新画的东西"，留到 P4.5 |
