...
| D-87 | **一笔 op 改三层时，方块先、稀疏层后** | 新增 `store.writeLayered(blocks, sparseOf, options)`，只推进一格版本；`sparseOf` 是"拿到写入结果之后才算得出"的回调 | §18.9；方块实体寄生于方块，`writeBlocks` 会顺手剪掉被写格子上的旧方块实体，而搬过来的新方块实体必须落在剪除**之后**。`commitSparse` 里原来那句"先落稀疏层再调 `writeBlocks`"在"同一格既改方块又改方块实体"时是错的 |
...
**闸门已经落地了一半**（D-89 的 `entity-textures.test.ts`）：漂移的表现曾经是"某个实体静默
变成兜底盒"或者"糊一层兜底灰"，现在这两条都会让测试红。**方块实体那张表仍然没有闸门**——
它的漂移表现是"新方块的附加数据写不进去"，而那是拒绝而不是静默，暂时接受。

**`.obj` 导出实体**：✅ 已定——**不导出**，理由与现状写进了 README。那条路只认
`registry.shapesOf(stateId)` 的碰撞盒，按 AABB 盒导出会给出一个"看着像、用起来错"的模型。
方块实体天然含在方块几何里，不受这条影响。

**复制与镜像要不要带上实体**：✅ 已定——带上，见 D-88。"同一个工程重放出来的结果会不会和以前不同"
这个顾虑不成立：重放是**按 op 里记下的差分**纯机械应用，老日志里没有那两层就是没有，
新行为只影响新写下的 op。
...
> 状态：**P1–P6 已落地**（三层数据模型 · 记账 · `.mcai` 两个新条目 · `extra` 写回 ·
> 两种交换格式的四个字段 · 五个工具与闸门 · 两条软件渲染路径 · three.js 视口与拾取 ·
> 搬运带上另外两层 · 稀疏层 linter 判据 · 打包）。
> 决策见 D-77…D-90。
>
> **唯一没能自证的**：`--gui-smoke` 的实体断言在本机跑不起来（Electron 起不来），
> 所以"桌面视口里真的画出了实体"要有一次真机确认。
...
| `.litematic` | ~~已固定塞**空** `Entities` 与 `TileEntities`，读侧忽略~~ ✅ 已落地 | 读 v1 与 v2+，写 v2 形状（见下） |
...
**P2 的 `.litematic` 落地记录**：写 v2 形状、读 v1 与 v2+。两处与 Sponge 不同的
约定，全部取自 Litematica 自己的序列化代码：**键是小写 `id`**（原版实体/方块实体的
NBT 就叫这个），实体位置是 `Pos` = double 列表，而**方块实体位置是 `x`/`y`/`z` 三个 int**
（`DataTypeUtils.putVec3i`），不是 Sponge 那种 `Pos` 数组。

为此把"世界 → 两层稀疏数据"的提取抽成了 `collect.ts`：两种格式在这里**完全一样**
（位置口径、`yaw`/`pitch` 换算、转不成 NBT 时要报出来），各写一份迟早会漂开，
而漂开的表现是"同一个世界导出成两种格式，实体位置差一点"。有一条测试专门盯这件事：
同一个世界导出成 `.schem` 与 `.litematic`，**读回来必须是同一份中间表示**。
...
**P4 落地记录**（`packages/render/src/`）：

- 实体模型表烘成 `data/<版本>/entitymodels.json`（94 个模型、401 KB），进 `bake:check`。
  它**其实不是版本相关的**（上游那张表对所有版本都用同一份），放进版本目录只是为了
  读取路径一致。
- **网格化器**：骨骼层级沿 `parent` 链复合、法线跟着转、最低点落到实体位置上。
  三处都与上游 `prismarine-viewer` 不同，理由见 `entitymesher.ts` 的文件头。
  朝向约定由测试钉死：`yaw=4` 必须逐顶点等于 `yaw=0` 绕 Y 转 90°。
- **图集是一张等大网格，tile 尺寸由内容决定**——plan 里原本写的是"货架装箱的异构
  tile"，实现时改掉了。理由：光栅器的"不透明 / 半透明"判据是"重心 UV → tile 序号
  → 查表"，那个查表**假设等大网格**；任意矩形会让重心落到毫不相干的格子上，判据
  变成随机（症状是"玻璃后面的东西时有时无"）。让 **tile 尺寸**随内容变，既让 128×64
  的船贴图进得来，又不动那条判据。只有船的场景是 128×128，来一条末影龙就是 256。
- **tile 允许比它装的贴图大**，所以逐 tile 记真实尺寸（`tileExtents`）：否则
  128×64 那条贴图的空下半部分（alpha=0）会把整条船判成半透明，船就丢了深度写入、
  自己的面互相不遮挡。
- **光栅器加了逐三角形材质位**（`WorldGeometry.materials` + `rasterize` 的
  `extraAtlases`）。两张图集的 tile 网格不一样，查错表就是随机的通道。透视路径上
  这个材质还要穿过近平面裁剪——一个三角形会变成几块，每一块都得查同一张图集。
- **实体图集按世界建、不按版本缓存**：需要哪些贴图取决于世界里有哪些实体。
- 纯色快路径把实体画成**碰撞盒**，与它画方块的方式一致；纹理路径画真模型，
  这是两条路径之间唯一的差异。
- **未做**：旗帜图案与告示牌文字。它们是 plan §18.6 里"方块实体渲染成本"表中
  唯一真需要新画的两样（其余要么方块状态已经画了、要么静态看不见）。这一期先把
  "实体本身能看见"钉死，那两样单列。

**P4 已知的两处近似**（要写进 README，而不是留在代码注释里）：
1. 实体贴图用的是 **alpha 裁剪**（原版 `alphaTest`），而我们的光栅器只有"不透明 /
   混合"两条路。P4 按不透明画：画布预填了背景色，所以透明像素看上去是背景，
   代价是那些像素**写了深度**，会挡住后面的东西。
2. 动画贴图（竖向帧条）**整张用**，没有取第一帧。方块图集取左上角 16×16，
   实体这条没做同样的事。

**P5 落地记录**（`apps/desktop/src/renderer/`、`main/services/studio.ts`）：

- `ScenePayload` 多带一层实体（`StudioState.entityCount`），渲染进程按"方块 + 实体"两段
  建网格，材质分不透明 / 半透明两条。
- 拾取拆出 `pickTriangle`，再叠一层 `pickEntity`（几何是三角形汤，靠 `owners` 找回身份）。
  这一步撞出两个**测试瞄错目标**的假失败：相机取景框以方块内容为先，船会偏出去约 20px；
  而空心船的船底与它下面的地板在深度上打平——真正选中它的是**船帮**。第一条测试还把船
  放进了小屋里，于是"墙正确地挡住了它"。
- 浮点位置改成浮动 overlay 盒（方块是整数格，实体在格内任意位置），左栏加实体列表。
- `--gui-smoke` 加了实体断言。**这一条始终没能在本机执行**：这个环境里 Electron 起不来
  （`--gui-smoke` / `--no-sandbox` / `--smoke` 都是一行输出都没有地退出 0，而脚本在成功时
  会逐条打印断言）。所以验收 ⑩ 与"视口里真的画出了实体"这两件事只有类型检查、
  payload 测试与打包构建作证，**真机确认要由用户来做**。

**P6 落地记录**（`core/src/world/`、`tools/src/tools/`、`render/src/`、README）：

- **打包**：`build.files` 那句 `data/*/*/**` 是总排除，后面只把 1.21.4 的 `blocks` 放了回来。
  开发机上 `node_modules` 就在旁边，所以一直没露出来；**装出来的 app 里放一条船读不到贴图**。
  补上 `entity` 与 `items`，并加了一条"运行时能请求到的纹理根都要在包里"的测试。
- 补那条测试时撞出**第二件事**：上游模型表的贴图路径**停在 1.16**。表里 207 条引用有 30 条
  在 1.21.4 里不存在，而失败方式是安静的——`buildEntityAtlas` 把读不到的路径收进 `missing`
  就继续跑，那个实体被画成"形状对、糊了一层兜底灰"。一只灰色的鱿鱼比一个灰盒子更难看出是坏的。
  三类修正（`entity-models.ts` 的 `MODEL_TEXTURE`）：路径挪窝（squid / arrow / player）、
  改画物品图标（烟花火箭 / 药水）、变体模型没有 `default` 的挑一个原版常见变体。
  命名空间口径也理了一次：`TexturePack` 对外统一用**资源包**的写法（`block/`、`item/`），
  `minecraft-assets` 的目录名（`blocks/`、`items/`）只在那一个来源里还原。
- **搬运带上另外两层**（`paste_region` / `symmetrize` / `run_batch` 里的 `paste_region`），
  见 D-87、D-88。
- **linter 加了五条判据**，见 D-90。
- 旗帜图案与告示牌文字**仍然没做**：数据存得住、导得出，只是截图里看不到那几个像素。
...
打包（**P6 已补**）：`apps/desktop/package.json` 的 `build.files` 里那句
`data/*/*/**` 是总排除，后面只放回了 `data/1.21.4/blocks`。开发机上 `node_modules` 就在旁边，
所以一直没露出来；装出来的 app 里放一条船读不到贴图。现在补上了 `entity`（2.2 MB）与
`items`（烟花火箭与药水那两个实体画的就是物品图标，见 D-89），并有一条测试盯着
"运行时代码能请求到的纹理根都要在包里"。
...
| **P4 软件渲染** ✅ | 实体网格化 + 实体图集 + 两条软件路径 + AABB 兜底盒 + golden | ⑧ 船的形状与朝向正确（golden 逐字节）；⑨ 既有 4 张 golden 逐字节不变 |
| **仍未做** | 旗帜图案、告示牌文字（见下面的记录） | 这两样是"方块实体里唯一需要新画的东西"，没有排期 |
| **P5 视口与交互** ✅ | `ScenePayload` + three 路径 + `pickTriangle` + overlay 浮点盒 + 左栏列表 | ⑩ `--gui-smoke` 新断言（**本机跑不了 Electron，未执行**）；⑪ 点选选中实体 |
| **P6 打磨** ✅ | `paste_region`/`symmetrize` 带实体、linter 判据、打包、README 的四处已知差异（i18n 已由 P2/P5 覆盖，本期无新串） | ⑫ linter 判据**写死**并配正反样例（附录 D 的教训） |
...
三件事在这里定下来，都是"另一个方向上的"——前八节讲的是怎么把三层放进去，
这一节讲的是它们**怎么跟着动**、怎么被检查出来。

**一笔 op 改三层（D-87）。** `store.writeLayered(blocks, sparseOf, options)`：先写方块，
再跑 `sparseOf` 落稀疏层，**只推进一格版本**。顺序不能反——方块实体寄生于方块，
`writeBlocks` 会顺手剪掉被写格子上的旧方块实体，而搬过来的新方块实体必须落在剪除
**之后**。`sparseOf` 是个回调而不是一个现成对象，因为它几乎总是取决于"哪些格子真的被写了"
（`mode: keep` 会跳过占着的格子、写同一份内容不算改动），而那个窗口正是"方块已落盘、
稀疏层还没落"。`run_batch` 把各 op 的意图按顺序拼起来，在**合并后的那一次写入**之后
统一落盘，整批仍然只有一个 revision。

**搬运带上另外两层（D-88）。**

| | 存什么 | 怎么搬 |
|---|---|---|
| 实体 | 去掉 `id` 的模板 + **浮点**局部坐标 | 点 `p → 2c+1-p`（symmetrize）或走 `transformLocalPointF`（paste）；`yaw` 与方块的 `rotation` 共用同一个 `remapRotationStep`，竖直镜像不翻 `yaw` 但把 `pitch` 取反；id 按新版本重新分配 |
| 方块实体 | 局部整数坐标 + kind + data | 先变换落点，再**落地后比对**那一格的 stateId 是否等于源方块变换后的结果——不相等就不搬（`keep` / `overlay` 会跳过占着的格子） |

三条语义上的取舍写在这里，因为它们在别处看不出来：

- **`only` 是方块名过滤器，所以它不顺走实体**。实体没有方块名可筛，带了的话"只要屋顶"
  会顺走停在屋顶上的船。
- **`clear: true`（默认）的 symmetrize 以源半区为准**：目标侧原有的实体会被删掉。
  `clear: false` 不删任何东西，只往镜像格是空气的地方补。
- **容量在写方块之前预检**（4096 / 65536）。写到一半才抛的话方块已经落盘而那一笔没有 op，
  撤销撤不掉、日志里也看不见——与 `place_entity` 的"整批先校验再动世界"同一条理由。

**体检（D-90）。** 五条判据单列，与方块那七类没有关系（那七类全从"每列 y 列表"推出来，
而实体是浮点对象、方块实体寄生于方块，套不进那套推导）：
...
| `entity_outside` | 实体所在格在分析范围**放宽 1 格**之外 | info |

两处判据本身的取舍：

- **不报"悬空"**。方块悬空是 error，因为一块砖没有支撑就是错的；而实体悬空是**常态**
  （箭、展示框、拴绳结、掉落的方块都在空中），豁免名单会随版本漂移，而漂移的表现是
  "把正常的东西报成错误"。`entity_embedded` 抓的是另一个方向、也明确得多的错误。
- **`entity_outside` 把范围放宽 1 格**（`LINT_ENTITY_OUTSIDE_MARGIN`）。默认范围是
  **方块**的内容包围盒，而一个站在建筑上的实体按定义就在它的上面一格——不放宽的话，
  每一只站在地板上的盔甲架都会被报成"跑到范围外去了"，而**一条只在正常场景里响的检查
  比没有还坏**。
