...
给用户用的**建筑风格模板**。选一个模板贴进对话框，改掉方括号里的内容即可。

> **这些是给用户的输入模板，不是系统提示词。** 系统提示词是英文的（D-11），
> 在 `packages/agent/src/prompts.ts` 里；用户看到的一切是中文。
>
> 为什么要模板：harness 里 LLM 的设计质量**很大程度取决于需求里有没有尺寸、材质、
> 特征物**。一句"造个房子"得到的是方盒子；下面这些模板把该说的都说了。

---

## 怎么写出好需求

四个要素，缺哪个模型就要自己猜（猜错的概率不低）：
...
再加一条**验收条件**（"门洞净高至少 2 格"这类），harness 会把它变成 `verify` 的检查项。
...
## 只改局部（最考验 harness 的一类）

一句"把门往左挪一格"是最容易出错的需求——模型看不见精确坐标。
这类需求**显式要求它先用 `slice` 看平面图**：
...
- **限定材质表**：想严格控制风格时，在项目设置里填 `paletteAllowlist`，
  系统提示词会把它变成硬约束（"只能用这些方块"）。
- **要求自检**：`每完成一个阶段就 verify 一次并说明检查了什么`。
  harness 的完成闸门本来就要求读回，明说只是让它早点做。
