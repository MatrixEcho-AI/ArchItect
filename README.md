...
![ArchItect 正在建造天安门城楼](docs/images/screenshot-tiananmen.jpg)
...
- **设计一座建筑：** 在右下角的输入框里描述需求，按 ⌘/Ctrl + Enter 发送。
...
- The model works the world through 29 LLM tools
...
- Place **entities and block entities** in the world, packed into the export formats
...
under 「模型配置」, then select 「测试连接」. The dialog then reports whether the model
...
- **Design a building:** describe it in the box at the bottom right and press ⌘/Ctrl +
  Enter.
- **Revisit a step:** drag the timeline, or select a row in 「编辑记录」. While the project
  is on a historical revision the send box is locked; return to the latest revision first.
- **Move the camera:** the viewport is a free camera — `WASD` to walk, Space to rise, Shift
  to descend, drag to turn, the wheel to zoom, double-click to fit. The toolbar's 「机位」
  list holds the isometric and elevation presets.
- **Open and save:** 「打开…」 and 「保存…」 in the toolbar read and write `.mcai` projects.
- **Export:** 「导出…」 in the toolbar is a three-way dropdown — `.schem` (WorldEdit),
  `.litematic` (Litematica), `.obj` (3D software).
- **Import:** 「导入…」 reads a `.schem` or `.litematic`, which can then be edited or
  exported.
...
| [`docs/tool-reference.md`](docs/tool-reference.md) | The complete reference for all 29 tools, generated from the JSON schemas |
| [`docs/prompt-library.md`](docs/prompt-library.md) | Request templates by building type: homes / public buildings / structure decoration / fixing problems |
| [`examples/README.md`](examples/README.md) | The example project, and how to regenerate it |
| [`plan.md`](plan.md) | The design document: world model, formats, rendering, tool semantics, agent loop, decisions |

### License

Copyright (C) 2026 ArchItect contributors

This project is released under **GPL v3**. It is free software: you may use, study, modify
and redistribute it, derivative works must be offered under the same license, and it comes
with no warranty.

Third-party components keep their own licenses, compatible with GPL v3.
