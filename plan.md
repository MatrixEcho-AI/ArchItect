...
- **视口是一台自由相机（D-75）**：`WASD` 平移（Shift 加速；W/S 沿视线前后，所以抬头按 W 就是上升），
  拖动是**原地转头**——角度绕相机自己的轴转、位置一动不动，不再是"绕着画面中心转"；
  滚轮缩放，双击回到自动取景。相机落了地（第一次转头/平移）之后取景倍率定住，
  不然转起来建筑会忽大忽小。**换工程/换预设/双击**都会回到自动取景。
...
| D-75 | **视口是一台自由相机：位置是真的，拖动 = 原地转头** | 交互相机持有 `eye`（世界坐标）+ 自己的角度（`renderer/freecamera.ts`）；**画面中心是推导出来的** `eye + focus·forward`。拖动只改角度、不动位置（`turn`），`WASD` 沿自己的轴平移（`moveStep`：W/S 走完整视线，A/D 走水平右方），第一次交互时 `settleCamera()` 把相机落到内容外面（`standingDistance()` = 包围球半径）并把取景倍率**定住**。仰角放开到 `±89`（`clampFreeElevation`，负值 = 抬头），模型那条路仍是 1..89 | 用户原话："不管怎么拖动，画面都是围绕中心旋转，不能更改视角面向的方向"。正交投影下成像只取决于"视线方向 + 画面正中是哪一点"，而旧实现里那个点是**钉死**的注视点（默认为内容中心）——于是拖动必然表现为"建筑原地自转"，而且没有位置概念，`WASD` 无从谈起。把位置变成真的之后，转头时画面中心转到新视线上 = 画面会平移，这才是"相对相机自己转"；`focus`（画面中心离相机多远）**不参与成像**，只是个任意常数，所以它不代表"离得多近"，只决定拾取射线起点与推给模型的 `lookAt`。推给模型的机位带上 `eye`：只推角度的话，抬头的负仰角会被模型那条路的 1..89 夹掉。附带修掉一个老 bug：滚轮第一次缩放从凭空的 `1` 起步（小屋自动取景约 14 px/格，滚一格缩成一个点），现在从 `fittedScale()` 起步 |
