...
export const zhCN = {
  ...
  viewport: {
    ...
    cam: {
      ...
      apply: '应用',
      reset: '复原',
      hint: '视口是第一人称透视：位置决定你在哪儿，角度与视场角决定你看到多少。视场角越小越像长焦。',
      applied: '机位已应用',
      shared: '模型将从这个机位截图',
      sharedShort: '模型同机位',
      unshared: '模型已回到默认机位',
  ...
  settings: {
    ...
    llm: {
      ...
      testing: '正在测试…',
      vision: '支持图像输入',
      discovered: '发现 {{count}} 个可用模型',
      ok: '连接正常：{{model}}，图像输入{{vision}}，单图约 {{imageTokens}} token',
      noVision: '不支持',
      yesVision: '支持',
      saved: '配置已保存（Key 不落盘到工程文件）',
      keyNote: 'API Key 只存在本机加密存储里，绝不写进 .mcai——那是一个可分享的文件。',
      keyPresent: '已保存（留空表示不改动）',
      imageTokenCost: '单图 token',
      contextWindow: '上下文窗口',
      capabilitySource: '能力来自运行时实测（GET /models + 探针），不读静态表',
    },
  },
  error: {
    ...
    OUT_OF_VOLUME: '{{count}} 个方块超出工区被裁剪',
  ...
  cli: {
    ...
    ops: {
      empty: '（没有编辑记录）',
      header: 'rev  id          tool                 source   changed   覆盖  clipped  时间',
    ...
    providers: {
      ...
      value: {
        ...
        cacheAuto: '（→ Regime A：历史只追加、截图不剪）',
        cacheNone: '（→ Regime B：截图按需剪枝）',
        sourceProbe: '本次实测',
      },
