...
两条路共用**同一份几何**（mesher 的输出）与**同一份相机**（`cameraBasis` / `fitCamera`），
所以视口里看到的和模型看到的是同一个世界，只是画法不同（D-48）。
...
> | 模型**设计得出来**一座可辨认的建筑 | 真模型 + 黄金任务 | ⏳ 等 SK |
>
...
| D-27 | **工具参考文档从 schema 生成，过期即测试失败** | `pnpm docs` 生成，`docs:check` 与 `pnpm test` 校验；说明文字逐字取自工具描述 | M8；手写文档必然漂移，而读到错契约的人不会知道 |
...
| D-47 | **交互视口用 three.js（WebGL），模型截图仍走软件光栅器** | 桌面渲染进程用 three.js 画方块（MSAA、mipmap、GPU 60 fps），叠加层仍是 2D 画布；主进程的软件光栅器保留给 CLI 与模型截图 | 两者的诉求相反：**交互要好看**（抗锯齿、纹理不闪、跟手），**截图要可复现**（CI 与 golden 测试没有 GPU）。早期为了统一而让交互走"每帧 IPC 拿 RGBA"，结果两头不讨好：没有抗锯齿、拖动还得降分辨率 |
